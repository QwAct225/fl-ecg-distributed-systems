...
def handle_client(conn, addr, global_model, client_models, client_data, client_index, cycle, metrics):
    ...
    try:
        ...
        received_length = 0
        print("Downloading client model:")

        while received_length < data_length:
            chunk = conn.recv(min(65536, data_length - received_length))  # Use conn, not s
            if not chunk:
                break
            ...
            # Update progress bar
            display_progress(received_length, data_length, "Downloading client model")

        print()  # Ensure we move to a new line after the progress bar
